...
class MiVOLO:
    ...
    def fill_in_results(self, output, detected_bboxes, faces_inds, bodies_inds):
        ...
        for index in range(output.shape[0]):
            ...
            current_obj_dict["face_ind"] = face_ind
            current_obj_dict["body_ind"] = body_ind
            ...
            if gender_probs is not None:
                ...
                current_obj_dict["gender"] = gender

                current_obj_dict["gender_score"] = gender_score

                detected_bboxes.set_gender(face_ind, gender, gender_score)
                detected_bboxes.set_gender(body_ind, gender, gender_score)

            current_obj_dict["face_boundings"] = detected_bboxes.get_bbox_by_ind(face_ind).tolist()
            current_obj_dict["body_boundings"] = detected_bboxes.get_bbox_by_ind(body_ind).tolist()

            self.result[index] = current_obj_dict
            cur_result[index] = current_obj_dict.copy()
        return cur_result
